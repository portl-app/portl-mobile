...
function SportPicker({ selected, onSelect }) {
  ...
  return (
    <View>
      ...
      >
        <Text style={{ color: selected ? "#FFFFFF" : "#666666", fontSize: 16 }}>{selected || "Select sport..."}</Text>
        <Text style={{ color: "#666666", fontSize: 14 }}>{open ? "▲" : "▼"}</Text>
      </TouchableOpacity>
      {open && (
        <View style={{ backgroundColor: "#F5F5F5", borderRadius: 8, marginTop: 4, borderWidth: 1, borderColor: "#E0E0E0" }}>
          {SPORTS.map((sport) => (
            <TouchableOpacity
              key={sport}
    ...
    <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: "#FFFFFF" }}>
      ...
      >
        <View style={{ marginBottom: 32 }}>
          ...
          <Text style={{ fontSize: 14, color: "#666666" }}>Set up your recruiting profile</Text>
        </View>

        <View style={{ gap: 20 }}>
          <View>
            <Text style={{ fontSize: 14, color: "#111111", marginBottom: 8 }}>Full Name *</Text>
            <TextInput value={formData.full_name} onChangeText={(v) => updateField("full_name", v)} placeholder="Enter your full name" placeholderTextColor="#666666" style={{ backgroundColor: "#F5F5F5", color: "#FFFFFF", padding: 16, borderRadius: 8, fontSize: 16 }} />
          </View>

          <View>
            <Text style={{ fontSize: 14, color: "#111111", marginBottom: 8 }}>School/Team *</Text>
            <TextInput value={formData.school} onChangeText={(v) => updateField("school", v)} placeholder="e.g. University of Alabama" placeholderTextColor="#666666" style={{ backgroundColor: "#F5F5F5", color: "#FFFFFF", padding: 16, borderRadius: 8, fontSize: 16 }} />
          </View>

          <View>
            <Text style={{ fontSize: 14, color: "#111111", marginBottom: 8 }}>Sport *</Text>
            <SportPicker selected={formData.sport} onSelect={(v) => { updateField("sport", v); updateField("recruiting_positions", []); }} />
          </View>

          <View>
            <Text style={{ fontSize: 14, color: "#111111", marginBottom: 8 }}>Contact Email *</Text>
            <TextInput value={formData.email} onChangeText={(v) => updateField("email", v)} placeholder="your.email@school.edu" placeholderTextColor="#666666" keyboardType="email-address" autoCapitalize="none" style={{ backgroundColor: "#F5F5F5", color: "#FFFFFF", padding: 16, borderRadius: 8, fontSize: 16 }} />
          </View>
          ...
          {positions.length > 0 && (
            <View>
              <Text style={{ fontSize: 14, color: "#111111", marginBottom: 8 }}>{recruitingLabel}</Text>
          ...
          <View>
            <Text style={{ fontSize: 14, color: "#111111", marginBottom: 8 }}>Recruiting Notes</Text>
            <TextInput
              ...
              multiline
              numberOfLines={4}
              style={{ backgroundColor: "#F5F5F5", color: "#FFFFFF", padding: 16, borderRadius: 8, fontSize: 15, height: 100, textAlignVertical: "top" }}
            />
          </View>
        </View>
        ...
        <TouchableOpacity
          ...
          disabled={loading}
          style={{ backgroundColor: "#000000", padding: 16, borderRadius: 8, alignItems: "center", marginTop: 32, opacity: loading ? 0.5 : 1 }}
        >
